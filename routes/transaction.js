                        ...
                        if(rows[0].discount != null){
                            db.query("SELECT name FROM discount WHERE id = ?", [rows[0].discount], (err, rows2, result) => {
                                if (err) {
                                    ...
                                    res.status(200).jsonp({data:{ id: rows[0].id, discountId: rows[0].discount, discountName: rows2[0].name, companyId: rows[0].companyId, companyName: rows[0].companyName, userName: rows[0].userName, userNurname: rows[0].userSurname, value: rows[0].value, date: rows[0].date.toISOString().split("T")[0]}, msg:"Transaction loaded."});
                                }
                            });
                        } else {
                            res.status(200).jsonp({data:{ id: rows[0].id, discountId:null, companyId: rows[0].companyId, companyName: rows[0].companyName, userName: rows[0].userName, userSurname: rows[0].userSurname, value: rows[0].value, date: rows[0].date.toISOString().split("T")[0]},msg:"Transaction loaded."});
                        }
                    } else {
                        ...
                        if(rows[0].discount != null){
                            db.query("SELECT name FROM discount WHERE id = ?", [rows[0].discount], (err, rows2, result) => {
                                if (err) {
                                    ...
                                    res.status(200).jsonp({data:{ id: rows[0].id, discountId: rows[0].discount, discountName: rows2[0].name, companyId: rows[0].companyId, companyName: rows[0].companyName, userName: rows[0].userName, userSurname: rows[0].userSurname, value: rows[0].value, date: rows[0].date}, msg:"Transaction loaded."});
                                }
                            });
                        } else {
                            res.status(200).jsonp({data:{ id: rows[0].id, discountId: null, companyId: rows[0].companyId, companyName: rows[0].companyName, userName: rows[0].userName, userSurname: rows[0].userSurname, value: rows[0].value, date: rows[0].date}, msg:"Transaction loaded."});
                        }
                    } else {
                        res.status(404).jsonp({msg:"Transaction not found!"});
                    }
            ...
            db.query("SELECT transaction.id AS id, user.surname AS surname, transaction.discount as discount, transaction.value as value, transaction.date as date FROM transaction INNER JOIN user ON user.id = transaction.user LEFT JOIN discount ON transaction.discount = discount.id WHERE transaction.company = ? ORDER BY transaction.date DESC LIMIT 20", [req.decoded.id], (err, rows, result) => {
                if (err) {
                    ...
                    if (rows[0]) {
                        ...
                        res.status(200).jsonp({data:transact, msg:"Last transactions loaded."});
                    } else {
